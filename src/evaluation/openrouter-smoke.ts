...
if (!validated.success) throw new Error(`Authenticated output failed local validation: ${validated.reasons.join(", ")}; output=${JSON.stringify(result.output)}`);
console.log(JSON.stringify({ authenticated: true, model: OPENROUTER_MODEL, provider: result.provider, usedZdr: result.usedZdr, latencyMs: result.latencyMs, inputHash: stableHash(context), outputHash: stableHash(validated.data), intent: validated.data.intent, payNowPaise: validated.data.pay_now_paise, promisedDate: validated.data.promised_date, disputeSignal: validated.data.dispute_signal }, null, 2));
