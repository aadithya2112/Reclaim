export const SIMULATOR_VERSION = "4.0.0";
...
export type StrategyName = "recoup-hybrid" | "recoup-agent" | "finance-age-bucket" | "razorpay-native-reminders" | "no-intervention";
...
export type DecisionFactorEffect = "SUPPORTS_ACTION" | "SUPPORTS_WAIT" | "REQUIRES_REVIEW";
export interface DecisionFactor { signal: string; value: string | number | boolean; effect: DecisionFactorEffect; explanation: string }
export interface RecoveryDecision {
  ...
  inferenceMode?: "DETERMINISTIC_KEYLESS" | "CACHED_EXTERNAL_MODEL";
...
export interface DecisionCacheEntry { cacheKey: string; inputHash: string; caseId: string; simulationDay: number; strategy: StrategyName; strategyVersion: string; inferenceMode: "DETERMINISTIC_KEYLESS" | "CACHED_EXTERNAL_MODEL"; decision: RecoveryDecision }
export interface DecisionCache { schemaVersion: "1.0.0"; evidenceLabel: "MEASURED_STRATEGY_DECISIONS"; sha256: string; entries: DecisionCacheEntry[] }
...
export interface WorkCandidate { caseId: string; action: RecoveryAction; capacityKind: CapacityKind; outstandingPaise: number; daysOverdue: number; hasBrokenPromise: boolean; contactAttempts: number; heuristicOpportunityScore?: number; priorityScore?: number }
