...
- Use a dedicated Neon Free-plan project in its Singapore AWS region. Give the web service a pooled TLS connection and the one-off Drizzle migration task a direct TLS connection. Treat the Free plan's compute, storage, transfer, restore-window, and network-isolation limits as demo constraints; store only synthetic and Razorpay Test Mode data.
- Keep authoritative DNS in Cloudflare, terminate the Cloudflare-to-origin connection at the load balancer with a DNS-validated non-exportable ACM certificate, and use Full (strict) TLS. Ensure the Razorpay webhook path is neither cached nor subjected to an interactive challenge.
- Deploy the current application without changing its bounded recovery, accounting, policy, or evidence semantics.
- Run schema migrations over Neon's direct connection as a controlled one-off task, verify Neon persistence across ECS service replacement, exercise health checks and scaling configuration, monitor Neon Free-plan consumption, and document rollback and teardown.
...
- Add a concise architecture diagram and judge-facing deployment evidence. Treat AWS as container-hosting and operational-readiness evidence and Neon as the disclosed managed persistence layer; do not claim private-database isolation, production SLA, or causal recovery evidence from this hackathon topology.

**Done when:** the AWS resources can be reproducibly provisioned, deployed, updated, observed, rolled back, and removed; the public judge journey passes; Neon data persists across a web-service redeploy; no-NAT outbound connectivity and pooled/direct database paths are verified; secrets remain server-side; and the signed hosted Test Mode webhook advances `INV-003` exactly once.
